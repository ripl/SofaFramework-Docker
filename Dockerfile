...
RUN echo 'export QTIFWDIR="/builds/Qt/Tools/QtInstallerFramework/3.0"' >> /etc/bash.bashrc
RUN echo 'export PYTHONPATH=/builds/build/master/lib/python3/site-packages:/builds/plugins/SofaPython3/splib:/builds/plugins/ModelOrderReduction/python:/builds/src/tools/sofa-launcher:/builds/plugins/STLIB/python3/src:/builds/plugins/SoftRobots/python3:$PYTHONPATH' >> /etc/bash.bashrc
RUN echo 'export PATH=/builds/build/master/bin:$PATH' >> /etc/bash.bashrc
RUN echo 'export PATH=$QTIFWDIR/bin:$PATH' >> /etc/bash.bashrc
RUN echo 'export XDG_RUNTIME_DIR=/run/user/1000' >> /etc/bash.bashrc
RUN echo 'export SOFA_ROOT=/builds/build/master/' >> /etc/bash.bashrc
