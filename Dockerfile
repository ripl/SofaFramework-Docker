...
RUN python2 -m pip install ipykernel plotly iplot

RUN python2 -m pip install pybind11 && python -m pip install "pybind11[global]"
...
RUN sudo echo 'export PYTHONPATH=/builds/build/master/lib/python/site-packages:/builds/plugins/ModelOrderReduction/python:/builds/src/tools/sofa-launcher:/builds/plugins/STLIB/python/src:/builds/plugins/SoftRobots/python:$PYTHONPATH' >> /etc/bash.bashrc
...
CMD /bin/bash -c "source ~/.bashrc && cd ~/workdir && /bin/bash "
#CMD "/usr/bin/Xorg -noreset +extension GLX +extension RANDR +extension RENDER -logfile ./xdummy.log -config /etc/X11/xorg.conf :1"
